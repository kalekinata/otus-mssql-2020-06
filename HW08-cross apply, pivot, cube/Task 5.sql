...
SELECT T.FolderId (id ïàïêè),
		   T.FileVersionId(id âåðñèè ïàïêè),
		   T.FileId	(id ôàéëà)	
...
/*В результате выполнения запроса выводятся по одному id папки, id версии файла и id файла для каждой папке, у которой RowNum = 1 и больший DirVersionId <= FHR.DirVersionId.
В соответствии с условиями, при которых FolderId и DirId должны совпадать с данными из таблицы, версия файла должна существовать и совпадать с данными из таблицы, и необходимо,
чтобы не существовало значений с FileId, FolderId, DirVersionId из сторонней таблицы при том, что вместе с этим не должно существовать значения с совпадающими FolderId, FileId,
FileVersionId из таблицы файловой истории с имеющимися на первом этапе таблицами.*/
